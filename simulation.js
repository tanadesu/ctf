...
export function command(r,p,a){
 ...
 case 'enter':{const m=r.markets[a.market];if(!m||p.access[a.market]!==1)throw Error('先に調査してください');if(!active(p,'logistics',r.elapsed).length)throw Error('稼働中の物流拠点が必要です');spend(p,m.entry,'investment');p.access[a.market]=2;break;}
 case 'trade':{const q=a.quote;if(!q||p.access[q.market]!==2)throw Error('市場へ参入してください');const m=r.markets[q.market],fresh=quote(m,q.good,q.side,q.n);if(fresh.total!==q.total||fresh.fees!==q.fees)throw Error('価格が変化しました。見積もりを更新してください');if(q.side==='buy'){if(p.inventory[q.good]+q.n>C.inventoryCap)throw Error('自社在庫の上限です');spend(p,q.total,'trade');p.inventory[q.good]+=q.n;m.stock[q.good]-=q.n;}else{if(p.inventory[q.good]<q.n)throw Error('自社在庫が不足しています');p.inventory[q.good]-=q.n;p.cash+=q.total;p.sales+=q.gross;p.costs.trade+=q.fees;m.stock[q.good]+=q.n;m.quota-=q.n;}m.revision++;m.reasonTTL=5;m.reason=q.side==='buy'?'企業の購入で在庫減少':'企業の販売で在庫増加';break;}
